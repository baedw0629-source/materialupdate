...
if 'db' not in st.session_state:
    ...
    if loaded_db: st.session_state.db = loaded_db
    else:
        st.session_state.db = {
...
with tab2:
    ...
    # 양식 다운로드 (Indentation 및 sheet_name 수정 완료)
    ...
    if uploaded:
        ...
        df_new = df_new_raw[available_cols].where(pd.notnull(df_new_raw[available_cols]), None).drop_duplicates(subset=conf["keys"], keep='first')
        ...
        if st.button("🚀 마스터 DB 최종 반영"):
            ...
            if success:
                st.toast("✅ 동기화 완료!"); st.download_button("📥 결과 받기", get_excel_bytes(st.session_state.db), f"updated_{datetime.now().strftime('%m%d')}.xlsx")
